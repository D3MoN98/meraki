<section id="checkout_wrapper">
	...
	<section id="checkout_body">
		<div>
			...
			<div class="cart_option">
				<div id="checkout_cart_body">
					<?php if(count($buynow->result()) == 0){ ?>
					<p>No items in your cart</p>
					<?php } foreach ($buynow->result() as $key) { ?>
					<div class="cart_item">
						<div>
						...
						<div>
							...
							<span>$<?= $key->price ?> <?= $key->color?> <?= $key->size == '0' ? '' : $key->size ?></span>
						</div>
						<!-- <div>
							<div>
		...
		<form action="<?= base_url('user/order/').$buynow->row()->product_id.'/true'?>" id="order_form" method='post'>
			<div>
				<div>
					<h2>Address</h2>
			...
			<input type="hidden" value="<?= $buynow->row()->size?>" name='size'>
		</form>
	</section>
</section>